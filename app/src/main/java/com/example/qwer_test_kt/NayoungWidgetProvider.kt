...
class NayoungWidgetProvider : AppWidgetProvider() {
    ...
    private fun updateAllWidgets(context: Context) {
        ...
        val componentName = ComponentName(context, NayoungWidgetProvider::class.java)
        ...
        for (id in widgetIds) {
            ...
            val views = RemoteViews(context.packageName, R.layout.nayoung_widget)
            ...
            val imageRes = R.drawable.nayoung
    ...
    fun scheduleNextUpdate(context: Context) {
        val intent = Intent(context, NayoungWidgetProvider::class.java).apply {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
            if (alarmManager.canScheduleExactAlarms()) {
                ...
                Log.d("NayoungWidget", "Scheduled exact alarm")
            } else {
                ...
                Log.d("NayoungWidget", "Scheduled inexact alarm (no exact alarm permission)")
            }
        } else {
            // For Android 11 and below, no permission check needed
            alarmManager.setExactAndAllowWhileIdle(
                AlarmManager.RTC_WAKEUP,
                triggerAtMillis,
                pendingIntent
            )
            Log.d("NayoungWidget", "Scheduled exact alarm (Android < 12)")
        }
    }
